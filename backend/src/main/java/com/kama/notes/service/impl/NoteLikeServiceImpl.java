...
public class NoteLikeServiceImpl implements NoteLikeService {
    ...
    private final NoteHotRankService noteHotRankService;
    ...
    public ApiResponse<EmptyVO> likeNote(Integer noteId) {
        ...
        try {
            ...
            // 增加笔记点赞数（Redis 计数，异步落库）
            initLikeCountIfAbsent(noteId);
            redisService.increment(NOTE_LIKE_COUNT_KEY + noteId, 1);
            ...
            applicationEventPublisher.publishEvent(new MessageEvent(this,
                    MessageBuilder.likeNote(userId, sender.getUsername(), note.getAuthorId(), noteId)));
    ...
    public ApiResponse<EmptyVO> unlikeNote(Integer noteId) {
        ...
        try {
            ...
            if (noteLike != null) {
                ...
                // 减少笔记点赞数（Redis 计数，异步落库）
                initLikeCountIfAbsent(noteId);
                redisService.decrement(NOTE_LIKE_COUNT_KEY + noteId, 1);
                // 更新热度
                noteHotRankService.updateNoteHotScore(noteId);
            }
    ...
    /**
     * 如果 Redis 中不存在该笔记的点赞数，则从数据库读取并初始化
     */
    private void initLikeCountIfAbsent(Integer noteId) {
        String key = NOTE_LIKE_COUNT_KEY + noteId;
        if (!redisService.exists(key)) {
            Note note = noteMapper.findById(noteId);
            if (note != null) {
                redisService.setIfAbsent(key, note.getLikeCount());
