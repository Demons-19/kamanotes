...
public class CollectionServiceImpl implements CollectionService {
        ...
        for (UpdateCollectionBody.UpdateItem collection : collections) {
            ...
            if ("create".equals(action)) {
                try {
                    ...
                    if (collectionMapper.countByCreatorIdAndNoteId(userId, noteId) == 0) {
                        // 笔记不存在，给笔记增加收藏量（Redis 计数，异步落库）
                        initCollectCountIfAbsent(noteId);
                        redisService.increment(NOTE_COLLECT_COUNT_KEY + noteId, 1);

                        // 发送收藏通知（异步事件解耦）
                        User sender = userMapper.findById(userId);
                        Note note = noteMapper.findById(noteId);
                        if (note != null && sender != null) {
                            applicationEventPublisher.publishEvent(new MessageEvent(this,
                                    MessageBuilder.collectNote(userId, sender.getUsername(), note.getAuthorId(), noteId)));
                        }
                    }
                    ...
                    // 更新热度
                    noteService.updateNoteHotScore(noteId);
                } catch (Exception e) {
            ...
            if ("delete".equals(action)) {
                try {
                    ...
                    if (collectionMapper.countByCreatorIdAndNoteId(userId, noteId) == 0) {
                        // 笔记不存在，给笔记减少收藏量（Redis 计数，异步落库）
                        initCollectCountIfAbsent(noteId);
                        redisService.decrement(NOTE_COLLECT_COUNT_KEY + noteId, 1);
                    }
                    // 更新热度
                    noteService.updateNoteHotScore(noteId);
                } catch (Exception e) {
    ...
    /**
     * 如果 Redis 中不存在该笔记的收藏数，则从数据库读取并初始化
     */
    private void initCollectCountIfAbsent(Integer noteId) {
        String key = NOTE_COLLECT_COUNT_KEY + noteId;
        if (!redisService.exists(key)) {
            Note note = noteMapper.findById(noteId);
            if (note != null) {
                redisService.setIfAbsent(key, note.getCollectCount());
