...
public class NoteServiceImpl implements NoteService {
    ...
    private NoteVO fillUserActions(NoteVO base){NoteVO vo=new NoteVO();BeanUtils.copyProperties(base,vo);vo.setAuthor(base.getAuthor());vo.setQuestion(base.getQuestion());NoteVO.UserActionsVO ua=buildUserActions(base.getNoteId());vo.setUserActions(ua);return vo;}
    private NoteVO.UserActionsVO buildUserActions(Integer noteId){NoteVO.UserActionsVO ua=new NoteVO.UserActionsVO();if(!requestScopeData.isLogin()||requestScopeData.getUserId()==null)return ua;Long uid=requestScopeData.getUserId();String likedKey=RedisKey.noteUserLiked(uid,noteId);String collectedKey=RedisKey.noteUserCollected(uid,noteId);Object likedCached=redisService.get(likedKey);Object collectedCached=redisService.get(collectedKey);Boolean liked=parseCachedBoolean(likedCached);Boolean collected=parseCachedBoolean(collectedCached);if(liked==null){liked=noteLikeService.findUserLikedNoteIds(uid,Collections.singletonList(noteId)).contains(noteId);redisService.setWithExpiry(likedKey,liked,600);}if(collected==null){collected=collectionNoteService.findUserCollectedNoteIds(uid,Collections.singletonList(noteId)).contains(noteId);redisService.setWithExpiry(collectedKey,collected,600);}ua.setIsLiked(liked);ua.setIsCollected(collected);return ua;}
    private Boolean parseCachedBoolean(Object cached){if(cached instanceof Boolean value)return value;if(cached instanceof String value)return Boolean.parseBoolean(value);return null;}
    ...
    private void fillRealtimeCounts(List<Note> notes){if(CollectionUtils.isEmpty(notes))return;List<String> keys=new ArrayList<>();for(Note note:notes){keys.add(LK+note.getNoteId());keys.add(CK+note.getNoteId());keys.add(MK+note.getNoteId());}List<Object> values=redisService.multiGet(keys);for(int i=0;i<notes.size();i++){Note note=notes.get(i);applyRealtimeCount(note.getNoteId(),note,values.get(i*3),values.get(i*3+1),values.get(i*3+2));}}
    private void fillRealtimeCounts(Note n){if(n==null)return;Object like=redisService.get(LK+n.getNoteId()),collect=redisService.get(CK+n.getNoteId()),comment=redisService.get(MK+n.getNoteId());applyRealtimeCount(n.getNoteId(),n,like,collect,comment);}
    private void applyRealtimeCount(Integer noteId,Note n,Object like,Object collect,Object comment){if(like!=null)n.setLikeCount(Integer.parseInt(String.valueOf(like)));if(collect!=null)n.setCollectCount(Integer.parseInt(String.valueOf(collect)));if(comment!=null)n.setCommentCount(Integer.parseInt(String.valueOf(comment)));}
